...
export default function Section2({products,categories,cooperatives}) {
    ...
    const currentElements =displayedProducts.slice(indexOfFirstElement,indexOfLastElement).filter(val=>{if(val.deletedAt == null)return val})
...
const coops = cooperatives.filter(val=>{if(val.deletedAt == null)return val})
const cats = categories.filter(val=>{if(val.deletedAt == null)return val})
